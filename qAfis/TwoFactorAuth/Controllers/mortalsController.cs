...
namespace TwoFactorAuth.Controllers
{
    public class mortalsController : Controller
    {
        ...
        public async Task<ActionResult> BulkCreate([Bind(Include = "")] mortal mortal/*, HttpPostedFileBase file*/)
        {
            ...
            {
                int imgnumber = i + 1;
                string imgval = "(" + imgnumber.ToString() + ").png";
            ...
            string apppath = System.IO.Path.Combine(Server.MapPath("~"), "images");
            string pathToImage = (System.IO.Path.Combine(apppath, "(1).png"));
            ...
            string pathToImage2 = (System.IO.Path.Combine(apppath, "(" + (range / 2).ToString() + ").png"));
            ...
            int num = rand.Next(900);
            string apppath3 = System.IO.Path.Combine(Server.MapPath("~"), "images");
            string pathToImage3 = (System.IO.Path.Combine(apppath, "(" + num.ToString() + ").png"));
        ...
        public async Task<ActionResult> ManCreate([Bind(Include = "Name, Filename")] mortal mortal)
        {
            ...
            for (int j = 0; j < 60; j++)
            {
                string pathToImageM1 = (System.IO.Path.Combine(apppath, "augmented/augmented ("+(j+1).ToString()+")"));

                for (int i = 0; i < 10; i++)
                {

                    string pathToImageM2 = (System.IO.Path.Combine(pathToImageM1, "("+(i+1).ToString()+").jpg"));
                    count = count + 1;
                    string pathToImageO = (System.IO.Path.Combine(apppath, "original/(" + count.ToString() + ").png"));
            ...
            return File(myExport.ExportToBytes(), "text/csv", "results(100).csv");
